...
#	$out="OK store-id=http://lytogame.squid.internal/" . $1;
	...
	$out="ERR";
        ...
        @set_awesome = m/[&?]set_awesome\=([^\&\s]*)/;
        @cpn = m/[&?]cpn\=([^\&\s]*)/;
        unless (-e "/tmp/@cpn"){
        open FILE, ">/tmp/@cpn";
        print FILE "@set_awesome";
        close FILE;
        }
        $out="ERR";

} elsif ($u=~ m/^https?\:\/\/.*youtube.*watch.*/){
        @watch = m/[&?]watch\=([^\&\s]*)/;
        @cpn = m/[&?]cpn\=([^\&\s]*)/;
        unless (-e "/tmp/@cpn"){
        open FILE, ">/tmp/@cpn";
...
} elsif ($u=~ m/^https?\:\/\/.*googlevideo.*\/videogoodput?.*/){
        @good = m/[&?]good\=([^\&\s]*)/;
        @cpn = m/[&?]cpn\=([^\&\s]*)/;
        unless (-e "/tmp/@cpn"){
        open FILE, ">/tmp/@cpn";
...
} elsif ($u=~ m/^https?:\/\/[a-zA-Z0-9\-\_\.]+\.googlevideo\.com\/videoplayback.*/){
	@itag = m/[&?](itag\=[0-9]*)/;
	@fexp = m/[&?](fexp\=[^\&\s]*)/;
	@cver =  m/[&?](cver\=[^\&\s]*)/;
	@range = m/[&?](range\=[^\&\s]*)/;
